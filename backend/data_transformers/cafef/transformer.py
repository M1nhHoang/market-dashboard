"""
CafeF Transformer - Transform CafeF crawler output to CrawlerOutput.

Converts CafeFCrawler output to EventRecord format for LLM pipeline.
"""
...
class CafeFTransformer(BaseTransformer):
    """
    Transform CafeF crawler output to CrawlerOutput format.
    ...
    def transform(self, raw_data: Dict[str, Any]) -> CrawlerOutput:
        ...
        Args:
            raw_data: Raw output from CafeFCrawler.run().to_dict()
        ...
        if not raw_data.get("success", False):
            logger.warning(f"[CafeFTransformer] Crawl failed, returning empty output")
        ...
        logger.info(f"[CafeFTransformer] Transformed {len(events)} items to events")
    ...
    def _transform_item(self, item: dict) -> Optional[EventRecord]:
        ...
        try:
            ...
            logger.error(f"[CafeFTransformer] Error transforming item: {e}")
            return None
